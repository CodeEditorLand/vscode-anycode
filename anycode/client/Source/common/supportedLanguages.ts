...
export class SupportedLanguages {
	...
	async getSupportedLanguages(): Promise<
		ReadonlyMap<Language, FeatureConfig>
	> {
		if (!this._tuples) {
				...
				if (language.info.suppressedBy) {
					...
					if (
						...
						language.info.suppressedBy.some((id) =>
							vscode.extensions.getExtension(id, true),
						)
					) {
						this._log.appendLine(
							`[CONFIG] ignoring ${
								language.info.languageId
							} because it is SUPPRESSED by any of [${language.info.suppressedBy.join(
								", ",
							)}]`,
						);
						continue;
	...
	async _readLanguageInfos(): Promise<ReadonlyMap<string, Language>> {
		type Contribution = {
			["anycodeLanguages"]: JSONAnycodeLanguage | JSONAnycodeLanguage[];
		};
		...
		for (const extension of vscode.extensions.allAcrossExtensionHosts) {
			...
			for (const lang of languages) {
				...
				const language = new Language(info, async () => {
					const grammar = await vscode.workspace.fs.readFile(
						vscode.Uri.joinPath(
	...
	private static async _readQueryPath(
		...
		if (paths.comments) {
			result.comments = decoder.decode(
				await vscode.workspace.fs.readFile(
					vscode.Uri.joinPath(extension.extensionUri, paths.comments),
				),
			);
		}
		if (paths.folding) {
			result.folding = decoder.decode(
				await vscode.workspace.fs.readFile(
					vscode.Uri.joinPath(extension.extensionUri, paths.folding),
				),
			);
		}
		...
		if (paths.locals) {
			result.locals = decoder.decode(
				await vscode.workspace.fs.readFile(
					vscode.Uri.joinPath(extension.extensionUri, paths.locals),
				),
			);
		}
		if (paths.outline) {
			result.outline = decoder.decode(
				await vscode.workspace.fs.readFile(
					vscode.Uri.joinPath(extension.extensionUri, paths.outline),
				),
			);
		}
		if (paths.references) {
