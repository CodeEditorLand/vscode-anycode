...
function _updateStatusAndInfo(
	...
	_statusItem.command = {
		title: "Learn More",
		command: "vscode.open",
		arguments: [vscode.Uri.parse("https://aka.ms/vscode-anycode")],
	};
}
...
async function _startServer(
	factory: LanguageClientFactory,
	context: vscode.ExtensionContext,
	supportedLanguagesInfo: SupportedLanguages,
	...
	const client = factory.createLanguageClient(
		"anycode",
		"anycode",
		clientOptions,
	);
	...
	const exclude = `{${[
		...Object.keys(
			vscode.workspace.getConfiguration("search", null).get("exclude") ??
				{},
		),
		...Object.keys(
			vscode.workspace.getConfiguration("files", null).get("exclude") ??
				{},
		),
	].join(",")}}`;
	...
	let size: number = Math.max(
		0,
		vscode.workspace
			.getConfiguration("anycode")
			.get<number>("symbolIndexSize", 500),
	);
	...
	const init = Promise.resolve(
		vscode.workspace
			.findFiles(
				langPattern,
				exclude /*unlimited to count the number of files*/,
			)
			.then(async (all) => {
					...
					vscode.window.withProgress(
						{
							...
							title: `Updating Index for '${doc.languageId}'...`,
						},
						() => Promise.race([initLang, initCancel]),
					);
