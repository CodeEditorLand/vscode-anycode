...
import { type IUsage, getDocumentUsages } from "./references";
...
class SuffixFilter {
	...
	update(suffixes: string[]) {
		...
		this._regex = new RegExp(
			`\\.(${Array.from(this._suffixes)
				.map(SuffixFilter._escapeRegExpCharacters)
				.join("|")})`,
			"i",
		);
	}
...
export class SymbolIndex {
	...
	private async _doUpdate(uris: string[], async: boolean): Promise<void> {
		if (uris.length !== 0) {
			...
			console.log(
				`[index] (${async ? "async" : "sync"}) added ${
					uris.length
				} files ${sw.elapsed()}ms (retrieval: ${Math.round(
					totalRetrieve,
				)}ms, indexing: ${Math.round(totalIndex)}ms) (files: ${uris.map(
					String,
				)})`,
			);
		}
	...
	async initFiles(_uris: string[]) {
		...
		console.log(
			`[index] added FROM CACHE ${
				persisted.size
			} files ${sw.elapsed()}ms, all need revalidation, ${
				uris.size
			} files are NEW, ${obsolete.size} where OBSOLETE`,
		);
	}
