...
function readAnycodeExtension(candidate) {
	...
	for (const lang of languages) {
		const queryInfo = {};
		for (const prop in lang.queryPaths) {
		...
		bucket.push({
			...
			wasmUri: `/${relative(
				base,
				join(candidate, "../", lang.grammarPath),
			)}`,
...
async function runTests() {
	const target = new URL(
		`http://localhost:${port}/anycode/server/src/common/test-fixture/client/test.html`,
	);
